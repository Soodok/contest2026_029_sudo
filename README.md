...
└── dist/
    └── vela.Databank.debug.1.16.88.rpk   # 构建产物（可直接安装体验）
```
...
```bash
adb push dist/vela.Databank.debug.1.16.88.rpk /data/local/tmp/app.rpk
...
| 真机 | 未涉及（未领取开发板）。应用按小米手环 9 的胶囊屏形态与方形屏形态做自适应布局，决赛阶段如获得样机再做真机回归 |
